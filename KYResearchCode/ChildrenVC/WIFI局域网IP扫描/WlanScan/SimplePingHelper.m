...
- (void)successPingWithHostName:(NSString *)address
{
	[self killPing];
	[target performSelector:sel withObject:[NSNumber numberWithBool:YES] withObject:self.address];
}

- (void)failPing:(NSString*)reason hostName:(NSString *)address
{
	[self killPing];
    [target performSelector:sel withObject:[NSNumber numberWithBool:NO] withObject:self.address];
}
...
- (void)endTime
{
	if (self.simplePing) { // If it hasn't already been killed, then it's timed out
        [self failPing:@"timeout" hostName:self.address];
	}
}
...
{
	[self failPing:@"didFailWithError" hostName:self.address];
}

- (void)simplePing:(SimplePing *)pinger didFailToSendPacket:(NSData *)packet error:(NSError *)error
{
	// Eg they're not connected to any network
	[self failPing:@"didFailToSendPacket" hostName:self.address];
}

- (void)simplePing:(SimplePing *)pinger didReceivePingResponsePacket:(NSData *)packet
{
    [self successPingWithHostName:self.address];
}
