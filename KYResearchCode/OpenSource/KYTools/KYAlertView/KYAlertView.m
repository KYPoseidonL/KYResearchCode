...
@property (nonatomic, assign) KYAlertViewStyle style;
...
+ (instancetype)showAlertViewWithStyle:(KYAlertViewStyle)style
    ...
    return [self showAlertViewWithStyle:style noticStyle:KYAlertViewNoticStyleClassic title:title detail:detail canleButtonTitle:canle okButtonTitle:ok callBlock:callBack];
}
...
+ (instancetype)showAlertViewWithTitle:(NSString *)title
    ...
    return [self showAlertViewWithStyle:KYAlertViewStyleSuccess title:title detail:detail canleButtonTitle:canle okButtonTitle:ok callBlock:callBack];
}
...
+ (instancetype)showAlertViewWithStyle:(KYAlertViewStyle)style
                      ...
                      canleButtonTitle:(NSString *)canle
                         okButtonTitle:(NSString *)ok
                              delegate:(id<KYAlertViewDelegate>)delegate {
    
    KYAlertView * temp = [self showAlertViewWithStyle:style title:title detail:detail canleButtonTitle:canle okButtonTitle:ok callBlock:nil];
    temp.KYAlertViewDelegate = delegate;
    return temp;
}
...
+ (instancetype)shared {
    ...
    static KYAlertView *alert;
    dispatch_once(&once, ^{
        alert = [[KYAlertView alloc] init];
    });
    return alert;
}
 ...
 */
- (void)setNoticStyle:(KYAlertViewNoticStyle)noticStyle {
...
- (void)layerInit {
    ...
    switch (self.noticStyle) {
        ...
        case KYAlertViewNoticStyleFace:
        {
        ...
        default:
        case KYAlertViewNoticStyleClassic: {
...
- (CAShapeLayer *)layerConfig {
    ...
    showLayer.lineWidth = _noticStyle == KYAlertViewNoticStyleClassic ? CLASSICLLINEWIDTH : FACELINEWIDTH;
    return showLayer;
}
...
- (void)controlsInit {
    ...
    _detailLabel  = [[UILabel alloc] initWithFrame:CGRectMake(x ,_titleLabel.bottom + 5, width, Detial_Font )];
...
- (void) addButtonTitleWithCancle:(NSString *)cancle OK:(NSString *)ok {
    ...
    CGFloat centerY = _detailLabel.bottom + Button_Size_Height / 2.0 + 5;
    ...
    if (flag) {
        ...
        _OkButton.center = CGPointMake(_detailLabel.center.x + Logo_View_Size/4, centerY);
        _canleButton.center = CGPointMake(_detailLabel.center.x - Logo_View_Size/4, centerY);
        _canleButton.bounds = CGRectMake(0, 0, Logo_View_Size/3, Button_Size_Height);
        _OkButton.bounds = CGRectMake(0, 0, Logo_View_Size/3, Button_Size_Height);
    }
    _OkButton.hidden = NO;
...
- (void)buttonClick:(UIButton *)sender {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        self.hidden = YES;
        if (_KYAlertViewDelegate && [_KYAlertViewDelegate respondsToSelector:@selector(alertViewClick:)] ) {
            [_KYAlertViewDelegate alertViewClick:sender.tag - TAG ];
        }
